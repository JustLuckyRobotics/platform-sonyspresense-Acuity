...
FRAMEWORK_DIR = join(
    PLATFORM_DIR, "bundles", "framework-arduinosonyspresense-Acuity"
)
SDK_DIR = join(
    PLATFORM_DIR, "bundles", "tool-arduinosonyspresensesdk-Acuity"
)
...
original_variant = board_config.get(
...
computed_libs = [File(join(libpath, item)) for item in libraries]
computed_libs.extend(["gcc", "m", "supc++_nano"])
...
if "windows" not in get_systype().lower() and not (
    set(["_idedata", "idedata"]) & set(COMMAND_LINE_TARGETS)
    and " " not in variant_dir
):
...
env.Append(
    ...
    LINKFLAGS=[
        ...
        "-nodefaultlibs",
        "-nostartfiles",
        "-Wl,--defsym,__reserved_ramsize=1572864-%d"
        % board_config.get("upload.maximum_size"),
        "-Wl,--gc-sections",
