...
BUNDLED_TOOLS_DIR = join(PLATFORM_DIR, "bundles", "tool-spresense-Acuity")
BUNDLED_SDK_DIR = join(PLATFORM_DIR, "bundles", "tool-arduinosonyspresensesdk-Acuity")

# for referencing the right precompiled tool in the bundled Spresense tools
os_folder_map = {
    "Windows": "windows",
    "Linux": "linux",
    "Darwin": "macosx"
...
env.Replace(
    ...
    SPKSIZEPRINTCMD='"%s" "$SOURCES"' % join(
        BUNDLED_TOOLS_DIR,
        "getspkinfo",
        "windows" if system() == "Windows" else os_folder_map.get(system(), "linux"),
        "getspkinfo.exe" if system() == "Windows" else "getspkinfo",
    ),
...
env.Append(
    BUILDERS=dict(
        ...
        ElfToSpk=Builder(
            action=env.VerboseAction(" ".join([
                join(BUNDLED_TOOLS_DIR, "mkspk", os_folder, "mkspk.exe" if system() == "Windows" else "mkspk"),
                "-c",
                "2",
                "$SOURCES",
                "$FLASH_NAME",
...
if "bootloader" in COMMAND_LINE_TARGETS:
    firmware_dir = join(BUNDLED_SDK_DIR, "firmware")
    fw_files = [
        "loader.espk",
        "gnssfw.espk",
        "dnnrt-mp.espk",
    ...
    bootloader_actions = [ 
        ...
        env.VerboseAction(" ".join([
            '"$PYTHONEXE"',
            '"%s"' % join(BUNDLED_TOOLS_DIR, "flash_writer", "scripts", "flash_writer.py"),
...
#
# Install wxPython dependency of the bundled Spresense upload tools
#
...
if {"upload", "erase", "bootloader"} & set(COMMAND_LINE_TARGETS):
    install_python_deps()
